...
(function () {
  ...
  Meteor.methods({
    ...
    async uploadImage({ roomName, nick, mimeType, b64Data }) {
      console.log("uploadImage", roomName, nick, mimeType, b64Data);
      check(this.userId, NonEmptyString);
      check(nick, NonEmptyString);
      check(roomName, NonEmptyString);
      check(mimeType, NonEmptyString);
      check(b64Data, NonEmptyString);
      const { id, name, webViewLink } = await driveEnv
        .get()
        ?.uploadImage(nick, mimeType, b64Data);
      await Messages.insertAsync({
        body: `${nick} uploaded an image to drive folder: ${webViewLink}`,
        system: true,
        room_name: roomName,
        bot_ignore: true,
        useful: true,
        timestamp: Date.now(),
        file_upload: { name, nick, mimeType, webViewLink, fileId: id },
      });
    },
  });
})();
